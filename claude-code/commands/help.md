...
Explain concisely that the YuE2 plugin can generate and plan songs, transcribe audio into ABC notation with SheetSage2, inspect/strip/compare scores, build listening comparison bundles, monitor durable tasks, and retrieve output artifacts. These operations run as server-side Runner tasks; the plugin contains no local processing scripts and never loads models on the client.
